 ...
 */
public class App {
    // Rentepercentages per rentevaste periode
    ...
    // Bereken het maximale te lenen bedrag op basis van inkomen
    public static double berekenMaxTeLenen(double inkomen, double partnerInkomen, boolean heeftStudieschuld) {
        ...
        if (heeftStudieschuld) {
            totaalInkomen *= 0.75; // 25% reductie vanwege studieschuld
        }
        return totaalInkomen;
    }

    // Bereken de maandlasten op basis van de lening en rentevaste periode
    public static double berekenMaandlasten(double lening, int rentevastePeriode) {
        ...
        double aflossing = lening / (30 * 12); // 30 jaar aflossing
        return renteBedrag + aflossing;
    }

    // Haal het bijbehorende rentepercentage op basis van de rentevaste periode
    ...
    // Controleer of een postcode uitgesloten is
